...
use std::sync::Arc; // for SocketAddr::from_str
...
fn main() -> Result<()> {
    ...
    match engine {
        ...
        Engine::sled => run_with_engine(SledKvsEngine::open(current_dir()?)?, pool, cli.addr),
    }
}
...
fn run_with_engine<E: KvsEngine, P: ThreadPool>(
    ...
    let server = KvsServer::new(engine, pool, stop);
    server.run(addr)
}
